...
export default function Feed({ page, userId }) {
  ...
  const isLoading = useFetchData({
    ...
    ...(page !== 'profile'
      ? { path: '/rooms', dependencies: [searchParams] }
      : {
          path: `/users/${encodeURIComponent(userId)}/rooms`,
  ...
  return isLoading ? (
    ...
    rooms.map((room, index) => (
      <div key={index} className="roomListRoom">
        ...
        <div className="roomListRoom__content">
          <Link to={`/room/${encodeURIComponent(room._id)}`}>{room.name}</Link>
          <p>{room.description}</p>
        </div>
        <div className="roomListRoom__meta">
          <Link
            to={`/room/${encodeURIComponent(room._id)}`}
